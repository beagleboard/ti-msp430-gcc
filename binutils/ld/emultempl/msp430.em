...
/* Given an input section S, which will not be present in the final linked
   executable due to a missing linker script rule to match it, warn the user
   that it has not been recognized if it is an interrupt vector section.
   GCC allows any string to be passed to the "interrupt" attribute, so the
   linker must perform the verification that the string corresponds to a
   real slot in the interrupt vector table.
   Retun TRUE if S is an interrupt vector section, to indicate there is
   nothing more to do.  */
static bfd_boolean
report_removed_interrupt_vector_section (asection *s)
{
  ...
  if (strncmp (s->name, vect_str, strlen (vect_str)) == 0)
    {
      einfo ("%P: warning: linker script %pU does not recognize interrupt "
				   ...
				   int constraint)
{
  ...
  /* An orphaned interrupt vector section means there is no matching rule for it
     in the linker script.  */
  if (report_removed_interrupt_vector_section (s))
    return NULL;
  ...
  /* If there isn't an output section beginning with .lower, use the
     un-prefixed output section.  */
  if (lower == NULL)
    lower = lang_output_section_find (name);

  if (lower == NULL)
    {
      /* If lower is still NULL, that means there's no .lower.X or .X output
	 section, which would be very strange.  The linker will create .text,
	 .data and .bss sections even if they are not named in the linker
	 script, but it won't create .rodata.  So this will only catch
	 .either.rodata.  */
      einfo (_("%P: warning: no section named %s or %s in linker script, "
	       "required to shuffle %s between regions\n"),
	     lower_name, name, s->name);
      goto end;
    }
  else if (upper == NULL)
    {
      /* We have a lower output section, but no upper.  */
      einfo (_("%P: warning: no section named %s in linker script, "
	       "required to shuffle %s between regions\n"),
	     upper_name, s->name);
      goto end;
    }
  ...
  free (upper_name);
  free (lower_name);
  return ldelf_place_orphan (s, (lower ? lower->name : secname), constraint);
}
EOF
fi
...
/* The same as insert_wild but with intermediate_relax_sections instead of
   lang_relax_sections.  */
...
add_wild_for_either (lang_statement_list_type *list)
{
  ...
  *(list->tail) = (void *)wild;
  ...
  /* We can't do regular lang_relax_sections during either shuffling, which is
     why we must use this variant of insert_wild that does
     intermediate_relax_sections instead.  */
	  ...
	  if (is == s)
	    {
	      ...
	      wild = add_wild_for_either (&new_os->children);

	      s->output_section = NULL;

	      if (wild)
		lang_add_section (&wild->children, s, NULL, new_os);
	      else
		einfo (_("%P: error: could not shuffle \"either\" section %s\n"), s->name);
		...
		*head = curr->header.next;
		...
		prev->header.next = curr->header.next;
		 ...
		 output section, then we also need to adjust that tail.  */
	      old_list = (lang_statement_list_type *)&old_os->children;
...
static void
check_interrupt_vector_used (bfd *abfd ATTRIBUTE_UNUSED,
			     asection *s, void *data ATTRIBUTE_UNUSED)
{
  /* If this section has been garbage collected, check if it is an interrupt
     vector section.  */
  if (s->gc_mark == 0)
    report_removed_interrupt_vector_section (s);
}
...
gld${EMULATION_NAME}_finish (void)
{
  ...
  /* If garbage collection is enabled, a missing linker script rule for an
     interrupt vector section will result in it being completely removed,
     rather than orphaned.  So that case needs to be handled here.  */
  if (link_info.gc_sections)
    {
      bfd *abfd;
      for (abfd = link_info.input_bfds; abfd != NULL; abfd = abfd->link.next)
	bfd_map_over_sections (abfd, check_interrupt_vector_used, NULL);
    }
}
