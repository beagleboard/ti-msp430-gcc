...
| Elf64:
|   typedef struct {
|     Elf64_Xword  smi_info;
|     Elf64_Xword  smi_value;
|   } Elf64_SymMetaInfo;
|
| The index of the symbol associated with the metasym, and the kind are
| extracted from the smi_info field.
|
| #define ELF32_SMI_IDX(i)	((Elf32_Word)(i) >> 8)
| #define ELF32_SMI_KIND(i)	((unsigned char)(i))
| #define ELF64_SMI_IDX(i)	((Elf64_Word)(i) >> 32)
| #define ELF64_SMI_KIND(i)	((unsigned char)(i))
|
| **** END NEW ****
			   ...
			   struct bfd_link_info *link_info)
{
  ...
  msym_buf = bfd_malloc (remaining_size);
  if (bfd_bread (msym_buf, remaining_size, ibfd) != remaining_size)
    goto error_return;

  bed = get_elf_backend_data (ibfd);

  /* Swap in the symbol meta-information entries and initialize the hash
     table.  */
  while (amt < remaining_size)
    {
      /* We're going to copy the data loaded into this temporary variable into
         the msym field of the metasym hash entry.  */
      Elf_Internal_SymMetaInfo msym;
      Elf_Internal_Sym *elfsym = NULL;

      bed->s->swap_metasym_in (ibfd, msym_buf + amt, &msym);
      amt += bed->s->sizeof_metasym;

      msym.index -= fix_index;
      if (msym.index >= symcount)
	goto error_return;

      sym = symbol_table[msym.index];
      ...
      else
	{
      ...
      /* Initialize the locsym tables if we are linking, so we can
       * start filling them with data.  */
      if (link_info && !bfd_link_relocatable (link_info)
	  && msym.kind == SMK_LOCATION)
	{
	  if (!bfd_elf_create_locsym_tables (ibfd, obfd))
	    {
	      _bfd_error_handler (_("%pB: failed to create locsym tables"),
				  obfd);
	      return FALSE;
	    }
	}
      ...
      if (!bfd_elf_add_metasym (obfd, ibfd, sym, elfsym, &msym))
	goto error_return;
    }
  free (msym_buf);
  return TRUE;
...
check_sec_for_locsym (bfd *abfd, asection *s)
{
  struct bfd_elf_metasym_hash_entry *ret = NULL;
  FOR_MSYM_IN_MSYM_HASH_TABLE (abfd, i, entry, msym)
    {
...
/* Verify locsyms are at their desired address and report if not.  */
void
bfd_elf_check_loc_metasyms (bfd *abfd, struct bfd_link_info *link_info)
{
  ...
  FOR_MSYM_IN_MSYM_HASH_TABLE (abfd, i, entry, msym)
	...
	continue;
      ...
      /* gc'ed sections will be reported in the map file, we don't need a
         special warning for the removal of a locsym.  */
      if (link_info->gc_sections && !msym->sym->section->gc_mark)
	continue;

      if (check_sec_for_locsym (abfd, msym->sym->section) != msym)
	{
	  /* We already warned for multiple locsyms in the same input
	     section.  */
	  continue;
	}
      ...
      else
	{
	  ...
	  if (isymbuf == NULL)
	    {
	      _bfd_error_handler (_("error: cannot find local symbol '%s' for SMK_LOCATION metasym"),
				  msym->sym->name);
	      return;
	    }
	  isym = isymbuf + msym->msym.index;
	  symval = (isym->st_value);
	}
      if (msym->msym.value != symval)
	_bfd_error_handler (_("warning: SMK_LOCATION metasym '%s' has VMA 0x%lx and has "
			      "not been placed at address 0x%lx"), msym->sym->name, symval,
			    msym->msym.value);
...
static bfd_boolean
write_locsym_tables (struct bfd_link_info *link_info, bfd *abfd)
{
  flagword flags;
  int dump = 0;
  bfd_vma pos = 0;
  asection *data_arr_sec = bfd_get_section_by_name (abfd, ".smi.location_init_array");
  /* There's no init_array section to write out.  */
  if (data_arr_sec == NULL)
    return TRUE;
  data_arr_sec = data_arr_sec->output_section;

  bfd_byte *data_contents = bfd_malloc (data_arr_sec->size);
  memset (data_contents, 0, data_arr_sec->size);


  FOR_MSYM_IN_MSYM_HASH_TABLE (abfd, i, hash_entry, msym)
    {
      asection *s = msym->sym->section;
	...
	continue;
      ...
      /* The locsym has been garbage collected.  */
      if (link_info->gc_sections && !msym->sym->section->gc_mark)
	continue;

      /* Use the properties of the input section, not the output section which
       * the locsym eventually gets placed in, as that might be a different
       * type of section. FOr example, a .bss variable might get placed in a
       * .text section, but we still need to zero that .bss variable.  */
      flags = msym->sym->section->flags;

      /* FIXME We don't need to create locsym table entries for all of these,
	 only for those which have their own output section.  That assumes the
	 others have been placed within compatible output sections.  */
      if ((flags & (SEC_LOAD | SEC_HAS_CONTENTS | SEC_THREAD_LOCAL)) == 0)
	{
	  if (dump)
	    printf ("%s is a symbol in the BSS section named %s\n", msym->sym->name,
		    msym->sym->section->output_section->name);
	  /* Add an entry in .smi.location_init_array.  */
	  /* For .bss variables we set the LMA to 0xFFFF so the startup code
	   * knows to set to 0 instead of copying from LMA to VMA.  */
	  lma = 0xFFFF;
	  vma = (s->output_offset + os->vma) & 0xFFFFFFFF;
	  size = s->size & 0xFFFFFFFF;
	  if (dump)
	    printf ("LMA 0x%x, VMA 0x%x, size 0x%x\n", lma, vma, size);
	}
      else if ((flags & SEC_READONLY) == 0)
	{
	  if (dump)
	    printf ("%s is a symbol in the DATA section named %s\n", msym->sym->name,
		    msym->sym->section->output_section->name);
	  /* Add an entry in .smi.location_init_array.  */
	  lma = (s->output_offset + os->lma) & 0xFFFFFFFF;
	  vma = (s->output_offset + os->vma) & 0xFFFFFFFF;
	  size = s->size & 0xFFFFFFFF;
	  if (dump)
	    printf ("LMA 0x%x, VMA 0x%x, size 0x%x\n", lma, vma, size);
	}
      /* FIXME if this a rodata variable gets placed in .data will it only be
       * if the region is directly loadable. i.e. do we need to create a locsym
       * table entry for these, since the value might need to be copied from
       * somewhere else?  */
      else if ((flags & SEC_CODE) == 0)
	{
	  if (dump)
	    printf ("%s is a symbol in the RODATA section named %s\n", msym->sym->name,
		    msym->sym->section->output_section->name);
	  /* Nothing to do for .rodata.  */
	  continue;
	}
      else
	{
	  if (dump)
	    printf ("%s is a symbol in the TEXT section named %s\n", msym->sym->name,
		    msym->sym->section->output_section->name);
	  /* Nothing to do for .text.  */
	  continue;
	}

      /* FIXME use swap_out functions.  Code is not portable at the
         moment.  */
      memcpy (data_contents + pos, &vma, 4);
      pos += 4;
      memcpy (data_contents + pos, &lma, 4);
      pos += 4;
      memcpy (data_contents + pos, &size, 4);
      pos += 4;
    }

  bfd_set_section_contents (abfd, data_arr_sec, data_contents, 0, data_arr_sec->size);
  return TRUE;
}
...
bfd_elf_write_symtab_meta (bfd *abfd, struct bfd_link_info *link_info)
{
  ...
  symtabmeta_hdr->sh_addralign = (bfd_vma) 1 << bed->s->log_file_align;

  /* Run through all the data and set the index of the corresponding symbol
     in the output symbol table at msym->msym.index, if it is not already set.  */
  FOR_MSYM_IN_MSYM_HASH_TABLE (abfd, i, hash_entry, msym)
    {
	  ...
	  if (msym->sym->flags & BSF_GLOBAL)
	    {
	      ...
	      h = elf_link_hash_lookup (elf_hash_table (link_info),
					msym->sym->name, FALSE, FALSE, FALSE);
	      if (h && h->indx > 0)
		msym->msym.index = h->indx;
	    }
	}
      else
	msym->msym.index = msym->sym->udata.i;
    }

  /* 21 is the size of .symtab_hash and .symtab_meta version.  */
  total_size = 21 + (elf_metasym_count (abfd) * bed->s->sizeof_metasym);

  contents = (bfd_byte *) bfd_malloc (total_size);
  if (contents == NULL)
    return FALSE;

  symtabmeta_hdr->contents = contents;
  if (elf_symtabmeta_version (abfd) == 0)
    elf_symtabmeta_version (abfd) = 2;

  /* Write out the symtab_meta version.  */
  H_PUT_8 (abfd, elf_symtabmeta_version (abfd), contents + amt++);
  ...
  /* Seek to the sh_offset of .symtab_meta so we can start writing out.  */
  FOR_MSYM_IN_MSYM_HASH_TABLE (abfd, i2, hash_entry2, msym2)
    {
      /* We haven't found the symbol, assume it's been removed.  */
      if (msym2->msym.index <= 0)
	{
	  total_size -= bed->s->sizeof_metasym;
	  continue;
	}

      bed->s->swap_metasym_out (abfd, &msym2->msym, contents + amt);
      amt += bed->s->sizeof_metasym;
    }
  ...
  if (abfd->is_linker_output)
    return write_locsym_tables (link_info, abfd);

  return TRUE;
}

/* Create .smi.location_init_array to instruct
   the startup code how to initialize disparate variables.
   We attach them to the input BFD so the orphan handling code will transfer
   them to the output BFD.  */
bfd_boolean
bfd_elf_create_locsym_tables (bfd *ibfd, bfd *obfd ATTRIBUTE_UNUSED)
{
  asection *s;
  static int done = 0;
  /* We only need to do this once per obfd, but we can't tell from the OBFD if
   * we've created the sections in an IBFD.  */
  if (done)
    return TRUE;

  /* Return if we've already create the tables.  They'll be filled after we
     write out .symtab_meta.  */
  if (bfd_get_section_by_name (ibfd, ".smi.location_init_array"))
    return TRUE;

  s = bfd_make_section_with_flags (ibfd, ".smi.location_init_array",
  ...
  s->flags |= SEC_KEEP;
  /* We need a NULL entry at the end of the table.  */
  bfd_set_section_size (s, 12);
			     ...
			     bfd_boolean copy)
{
  ...
  /* A new entry, since the sym field has not been initialized.  */
  if (msym->sym == NULL)
    {
      table->count++;
      return msym;
  ...
  /* There exists a hash table entry with the same name or hash already.
  ...
  for (tsym = msym; tsym != NULL; tsym = tsym->next)
    {
	  ...
	  && tsym->bfd_id == bfd_id
	  /* We create multiple hash table entries for different kind.
	     FIXME: This isn't very efficient and each hash table entry
	     should instead contain an array of (kind, value) pairs.  */
	  && tsym->msym.kind == kind)
	return tsym;
...
static bfd_boolean __attribute__((noinline))
add_locsym_table_entry (bfd *abfd)
{
  asection *s = bfd_get_section_by_name (abfd, ".smi.location_init_array");
  if (s == NULL)
    return FALSE;

  bfd_set_section_size (s, s->size + 12);
...
bfd_elf_add_metasym (bfd *abfd,
		     ...
		     Elf_Internal_SymMetaInfo *msym)
{
  struct bfd_elf_metasym_hash_entry * msym_h;
  if (ibfd == NULL)
    ibfd = abfd;

  if (msym->kind != SMK_RETAIN && msym->kind != SMK_LOCATION)
    return FALSE;
  ...
  else if (msym->kind == SMK_LOCATION)
    {
      FOR_MSYM_IN_MSYM_HASH_TABLE (ibfd, i, hash_entry, m)
	{
	  if (m->msym.kind == SMK_LOCATION
	      && m->msym.value == msym->value)
	    {
	      _bfd_error_handler
		(_("warning: ignoring SMK_LOCATION metasym '%s' with the same "
		   "value '0x%lx' as '%s'\n"), sym->name, msym->value,
		 m->sym->name);
	      /* Don't error.  */
	      return TRUE;
  ...
  msym_h = bfd_elf_metasym_hash_lookup (&elf_tdata(abfd)->metasym_hash_table,
				      sym->name, elfsym, ibfd->id, msym->kind,
				      TRUE, FALSE);
  if (msym_h == NULL)
    return FALSE;

  msym_h->sym = sym;
  msym_h->msym.kind = msym->kind;
  msym_h->msym.value = msym->value;
  ...
  msym_h->msym.index = msym->index;
  msym_h->elfsym = elfsym;
  msym_h->bfd_id = ibfd->id;

  if (msym->kind == SMK_LOCATION)
    add_locsym_table_entry (ibfd);
  return TRUE;
}
...
bfd_elf_copy_metasyms (bfd *obfd, bfd *ibfd)
{
  ...
  /* XXX Just copying the pointer to symtabmeta_hdr works for objcopy but
     ...
     other programs.  Likewise for copying the asymbol from msym_h->sym.  */
  elf_symtabmeta_hdr (obfd) = elf_symtabmeta_hdr (ibfd);

  FOR_MSYM_IN_MSYM_HASH_TABLE (ibfd, i, hash_entry, msym_h)
    if (!bfd_elf_add_metasym (obfd, ibfd, msym_h->sym, msym_h->elfsym, &msym_h->msym))
      return FALSE;
  return TRUE;
}
