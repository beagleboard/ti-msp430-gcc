...
struct msp430_cpu_state
{
  ...
  /* If "_sbrk_heap" is defined in the program, then the address of the end of
     heap is stored in memory at this address.  */
  bfd_vma heap_end;

  /* If "_sbrk_heap" is undefined, then the address of "_end" is the minimal SP
     value.  */
  bfd_vma data_end;
};
