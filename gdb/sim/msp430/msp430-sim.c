...
msp430_initialize_cpu (SIM_DESC sd, SIM_CPU *cpu)
{
  ...
  /* Assume pointer size is 16-bits unless the object attributes indicate
     otherwise.  */
  MSP430_CPU (sd)->state.ptr_size = 2;

  /* Load object attribute info.  */
  if (sd->base.prog_bfd)
    {
      attr = elf_known_obj_attributes_proc (sd->base.prog_bfd);
      if (attr &&
	  attr[OFBA_MSPABI_Tag_Code_Model].i == OFBA_MSPABI_Val_Code_Model_LARGE)
	MSP430_CPU (sd)->state.ptr_size = 4;
    }
}
		     ...
		     char * const *env)
{
  ...
  int c;
  int new_pc;
  asymbol **asymbols = STATE_PROG_SYMS (sd);
  int i;
  ...
  /* FIXME GDB segfaults trying to examine the symbols.  Maybe MSP430 debug
     information is corrupted.  */
  if (strcmp (STATE_MY_NAME (sd), "gdbsim") != 0)
    {
      /* In a non-standard program, _sbrk_heap and _end might not be defined,
	 so just disable the stack checking functionality in that case.  */
      MSP430_CPU (sd)->state.heap_end = MSP430_CPU (sd)->state.data_end = (bfd_vma) -1;

      /* Find the symbol that the simulator will used to catch stack overflow.
	 If "_sbrk_heap" isn't defined, the program doesn't use sbrk to
	 allocate heap space, or uses an old version of Newlib.
	 In that case, the address of the global "_end" symbol indicates the end of
	 the space allocated for sections defined in the program, and therefore the
	 minimum allowable SP value.  */
      for (i = 0; i < STATE_PROG_SYMS_COUNT (sd); i++)
	{
	  asymbol *sym = asymbols[i];
	  if (strcmp (sym->name, "_sbrk_heap") == 0
	      && sym->flags & BSF_GLOBAL)
	    {
	      /* If it exists, this symbol is always preferred as the marker for
		 the minimum SP over "_end".  */
	      MSP430_CPU (sd)->state.heap_end
		= sym->value + sym->section->vma;
	      break;
	    }
	  else if (strcmp (sym->name, "_end") == 0
		   && sym->flags & BSF_GLOBAL)
	    MSP430_CPU (sd)->state.data_end = sym->value + sym->section->vma;
	}
    }
...
trace_reg_put (SIM_DESC sd, int n, unsigned int v)
{
  if (n == 1)
    {
      /* If there is a minimum value for the stack pointer, check to
	 see if this operation will result in stack overflow.  */
      bfd_vma min_sp_val = (bfd_vma) -1;

      if (MSP430_CPU (sd)->state.heap_end != (bfd_vma) -1)
	{
	  unsigned char buf[4];

	  sim_core_read_buffer (sd, MSP430_CPU (sd), read_map, buf,
				MSP430_CPU (sd)->state.heap_end,
				MSP430_CPU (sd)->state.ptr_size);

	  if (MSP430_CPU (sd)->state.ptr_size == 4)
	    min_sp_val = buf[0] | (buf[1] << 8) | (buf[2] << 16) | (buf[3] << 24);
	  else
	    min_sp_val = buf[0] | (buf[1] << 8);
	}
      else if (MSP430_CPU (sd)->state.data_end != (bfd_vma) -1)
	min_sp_val = MSP430_CPU (sd)->state.data_end;

      if (min_sp_val != (bfd_vma) -1
	  && v < min_sp_val)
	{
	  /* FIXME add details of of where it happened.  */
	  fprintf (stderr, "STACK OVERFLOW, minimum SP value of 0x%lx is "
		   "breached with 0x%x\n", min_sp_val, v);
	  exit (1);
	}
    }
...
get_op (SIM_DESC sd, MSP430_Opcode_Decoded *opc, int n)
{
  ...
  switch (op->type)
      ...
      if (op->reg != MSR_None)
	{
	  ...
	  /* An MSP430 instruction with register value < 0x10000 will have bits
	     bits 19:16 cleared.  The full range of lower memory can be
	     addressed.  It is not necessarily an error if the address
	     wraps.  */
	  if (reg < 0x10000 && !opc->ofs_430x)
	    {
	      if (addr >= 0x10000)
		TRACE_MEMORY (MSP430_CPU (sd), " XXX WRAPPING ADDRESS %x on write, reg value %x", addr, reg);
	  ...
	  || (addr >= 0x4C0 && addr <= 0x4EB))
	{
	  switch (addr)
	    ...
	    default:
	      TRACE_MEMORY (MSP430_CPU (sd), "unimplemented HW MULT read from %x!", addr);
	  ...
	  || (addr >= 0x4C0 && addr <= 0x4EB))
	{
	  ...
	  switch (addr)
		...
		case UNSIGN_32:
		  ...
		  /* We need to perform unsigned 32-bit multiplication on
		     16-bit operands, which could be signed.  So we must cast
		     them to unsigned32 before multiplying, to ensure the result
		     is not implicitly sign-extended.  */
		...
		case UNSIGN_MAC_32:
		  a = HWMULT (sd, hwmult_op1);
		  b = HWMULT (sd, hwmult_op2);
		  HWMULT (sd, hwmult_accumulator) += (unsigned32) a * (unsigned32) b;
		...
		{
		case UNSIGN_64:
		  /* We need to perform unsigned 64-bit multiplication on
		     32-bit operands, which could be signed.  So we must cast
		     them to unsigned64 before multiplying, to ensure the result
		     is not implicitly sign-extended.  */
	    ...
	    default:
	      TRACE_MEMORY (MSP430_CPU (sd), "unimplemented HW MULT write to %x!", addr);
