...
;; We get the length by examining the type and number of operands.
...
;;
;; It is important to set either "type" or "length" attributes on any insn with
;; only one RTX operand.  Since the default "type" is "double", there is
;; a risk of a segfault trying to calculate the default length or extension
;; and examining a non-existent op1.  Since extension is only used to determine
;; the length, defining the length attribute means the default extension won't
;; be calculated.
...
; It looks like we should be able to allow volatile memory for op1, but there
; is no observable effect on code size for msp430 benchmarks, so we leave it.
(define_insn "movqihi"
...
(define_insn "movqi"
  [(set (match_operand:QI 0 "msp430_general_dst_operand" "=rYs,rm")
	(match_operand:QI 1 "msp430_general_operand" "riYs,rmi"))]
  ""
  "@
  MOV.B\t%1, %0
  * return (msp430_430x_insn_not_required (operands[0], operands[1]) ? \
	    \"MOV.B\t%1, %0\" : \
	    \"MOVX.B\t%1, %0\");"
  [(set_attr "type" "double")]
)

; FIXME this should be unnnecessary once combine deal with
...
(define_insn "movhi"
  [(set (match_operand:HI 0 "msp430_general_dst_operand" "=r,rYs,rm")
	(match_operand:HI 1 "msp430_general_operand" "N,riYs,rmi"))]
  ""
  "@
  MOV.B\t%1, %0
  MOV.W\t%1, %0
...
;; Some MOVX.A cases can be done with MOVA, this is only a few of them. FIXME
(define_insn "movpsi"
...
(define_insn "addqi3"
  [(set (match_operand:QI          0 "msp430_general_dst_operand" "=rYs,rm")
	(plus:QI (match_operand:QI 1 "msp430_general_operand" "%0,0")
		 (match_operand:QI 2 "msp430_general_operand"      "riYs,rmi")))]
  ""
  "@
   ADD.B\t%2, %0
   * return (msp430_430x_insn_not_required (operands[0], operands[2]) ? \
	     \"ADD.B\t%2, %0\" : \
	     \"ADDX.B\t%2, %0\");"
  [(set_attr "type" "triple")]
)
...
(define_insn "addhi3"
  [(set (match_operand:HI           0 "msp430_general_dst_operand" "=rYs,rm")
	(plus:HI (match_operand:HI  1 "msp430_general_operand" "%0,0")
		  (match_operand:HI 2 "msp430_general_operand"      "riYs,rmi")))]
  ""
  "@
   ADD.W\t%2, %0
   * return (msp430_430x_insn_not_required (operands[0], operands[2]) ? \
...
(define_insn "addsi3"
  [(set (match_operand:SI 0 "msp430_general_dst_nonv_operand" "=&r,rm")
...
(define_insn "addhi3_cy"
  [(set (match_operand:HI          0 "msp430_general_dst_operand" "=r,rm")
	(plus:HI (match_operand:HI 1 "msp430_general_operand" "%0,0")
		 (match_operand:HI 2 "msp430_nonimmediate_operand" "r,rm")))
   (set (reg:BI CARRY)
	(truncate:BI (lshiftrt:SI (plus:SI (zero_extend:SI (match_dup 1))
					   (zero_extend:SI (match_operand:HI 3 "msp430_nonimmediate_operand" "r,rm")))
				  (const_int 16))))
   ]
  ""
  "@
...
(define_insn "addchi4_cy"
  [(set (match_operand:HI                   0 "msp430_general_dst_operand" "=r,rm")
	(plus:HI (plus:HI (match_operand:HI 1 "msp430_general_operand" "%0,0")
			  (match_operand:HI 2 "msp430_general_operand"      "ri,rmi"))
...
;; Alternatives 2 and 3 are to handle cases generated by reload.
(define_insn "subqi3"
  [(set (match_operand:QI           0 "msp430_general_dst_nonv_operand"	"=rYs,  rm,  &?r, ?&r")
...
;; Alternatives 2 and 3 are to handle cases generated by reload.
(define_insn "subhi3"
  [(set (match_operand:HI           0 "msp430_general_dst_nonv_operand"	"=rYs,  rm,  &?r, ?&r")
...
(define_insn "bic<mode>3"
  [(set (match_operand:QHI                   0 "msp430_general_dst_operand" "=rYs,rm")
	(and:QHI (not:QHI (match_operand:QHI 1 "msp430_general_operand"       "rYs,rmn"))
...
(define_insn "and<mode>3"
  [(set (match_operand:QHI 0 "msp430_general_dst_operand" "=r,rYs,rm")
	(and:QHI (match_operand:QHI 1 "msp430_general_operand" "%0,0,0")
		 (match_operand:QHI 2 "msp430_general_operand" "N,riYs,rmi")))]
  ""
  "@
   AND%x0.B\t%2, %0
   AND%x0%b0\t%2, %0
...
(define_insn "ior<mode>3"
  [(set (match_operand:QHI          0 "msp430_general_dst_operand" "=rYs,rm")
	(ior:QHI (match_operand:QHI 1 "msp430_general_operand" "%0,0")
		 (match_operand:QHI 2 "msp430_general_operand" "riYs,rmi")))]
  ""
  "@
   BIS%x0%b0\t%2, %0
   * return (msp430_430x_insn_not_required (operands[0], operands[2]) ? \
...
;; NEW msp430 combiners
;; We get into big trouble wrt to code size when a pointer is stored in memory (or on the stack).
;; we need to try and avoid that at all costs
;; WE started with:
;; (set (reg:PSI)
;;     (sign_extend:PSI (subreg:HI (mem/c:QI (symbol_ref:PSI)) 0)))
;; Note paradoxical subreg as the rvalue
;; the low-order bits of the source value are stored
;; in reg and the high-order bits are discarded. When used as an
;; rvalue, the low-order bits of the subreg are taken from reg while
;; the high-order bits may or may not be defined.
;; The high-order bits of rvalues are defined in the following circum-
;; stances:
;; - subregs of mem When m2 is smaller than a word, the macro
;; LOAD_EXTEND_OP, can control how the high-order bits are de-
;; fined.
;; FIXME: Ideally a lot of these complicated patterns wouldn't be necessary,
;; if:
;; sign_extend:PSI (zero_extend:QHI) didn't happen
;; subreg:HI(mem:QI) wasn't searched for by combine
;; the relationship between PSI and the smaller modes still isn't properly
;; understood by the middle end, maybe we are missing some target macros,
;; or they aren't defined fully
...
;;
;; We avoid emitting insns in msp430_expand_shift, since we would have to
...
; Conversion of an integer to an __int20 pointer is not very well optimized
; and requires a lot of instructions.
; Also, each element of the jump-table requires 4 bytes with -mlarge, which adds
; to the code bloat.
; Even though fpic/pie is not properly supported, tests seemed to work before
; adding tablejump, so to keep the status quo we disable tablejump if pic/pie
; are used.
...
(define_insn "cbranchqi4_real"
  ...
  "@
   CMP.B\t%2, %1 { J%0\t%l3
   * return (msp430_430x_insn_not_required (operands[1], operands[2]) ? \
	     \"CMP.B\t%2, %1 { J%0\t%l3\" : \
	     \"CMPX.B\t%2, %1 { J%0\t%l3\");"
  [(set_attr "extra_length" "2")
   (set_attr "type" "cmp")]
...
(define_insn "*bitbranch<mode>4"
  [(set (pc) (if_then_else
	      (ne (and:QHI (match_operand:QHI 0 "msp430_general_dst_operand" "rYs,rm")
			   (match_operand:QHI 1 "msp430_general_operand" "rYsi,rmi"))
		  (const_int 0))
...
(define_insn "*bitbranch<mode>4"
  [(set (pc) (if_then_else
	      (eq (and:QHI (match_operand:QHI 0 "msp430_general_dst_operand" "rm")
			   (match_operand:QHI 1 "msp430_general_operand" "rmi"))
		  (const_int 0))
	      (label_ref (match_operand 2 "" ""))
	      (pc)))
   (clobber (reg:BI CARRY))
...
(define_insn "*bitbranch<mode>4"
  [(set (pc) (if_then_else
	      (ne (and:QHI (match_operand:QHI 0 "msp430_general_dst_operand" "rm")
			   (match_operand:QHI 1 "msp430_general_operand" "rmi"))
		  (const_int 0))
	      (pc)
	      (label_ref (match_operand 2 "" ""))))
...
; If we don't have hardware multiply support, it will generally be slower to
; call the mspabi library function to perform the widening multiplication than
; just leaving GCC to widen the arguments itself.  So unless we are optimizing
; for size we don't use the helper function in this case.
