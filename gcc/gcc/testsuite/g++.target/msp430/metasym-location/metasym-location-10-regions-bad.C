...
/* { dg-message "var_data_in_bss.*overlaps" "" { target { { ! msp430_region_upper } && { ! msp430_tiny_printf } } } 0 } */
int __attribute__((location(0x3002))) var_data_in_rodata = 12;
/* This can't be placed because data is incompatible with the .crt* section
   at the beginning of .text placed at 0x4000.  */
int __attribute__((location(0x4002))) var_data_in_text = 13;
/* { dg-message "var_data_in_text.*overlaps" "" { target { *-*-* } } 0 } */
